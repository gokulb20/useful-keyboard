...
final class AppController: NSObject {
    ...
    init(runtime: RuntimePaths, dictationStore: DictationStore? = nil) {
        ...
        self.dictationStore = dictationStore ?? DictationStore(
            databaseURL: AppPaths.defaultDatabaseURL(appName: AppIdentity.supportDirectoryName)
        )
    ...
    func start() {
        ...
        dataDidChangeObserver = DistributedNotificationCenter.default().addObserver(
            forName: AppNotifications.dataDidChange,
            object: nil,
            queue: .main
        ) { [weak self] _ in
            Task { @MainActor [weak self] in
    ...
    func signInWithChatGPT() async -> String? {
        do {
            ...
            fputs("[useful-keyboard] ChatGPT sign-in failed: \(error)\n", stderr)
    ...
    private func resummarize(
        ...
        Task { [weak self] in
            ...
            do {
                ...
                fputs("[useful-keyboard] failed to persist meeting summary: \(error)\n", stderr)
                await MainActor.run {
    ...
    func startMeetingRecording(title: String = "Meeting") {
        ...
        Task { [weak self] in
            ...
            do {
                ...
                fputs("[useful-keyboard] failed to start meeting: \(error)\n", stderr)
    ...
    func discardMeetingRecording() {
        ...
        statusBarController?.refresh()
        syncAppState()
        fputs("[useful-keyboard] meeting recording discarded\n", stderr)
    }
    ...
    func stopMeetingRecording() {
        ...
        Task { [weak self] in
            ...
            do {
                ...
                fputs("[useful-keyboard] meeting transcription failed: \(error)\n", stderr)
    ...
    private func cleanupTemporaryDirectory(named directoryName: String, logDescription: String) {
        ...
        if !files.isEmpty {
            fputs("[useful-keyboard] cleaned up \(files.count) \(logDescription)\n", stderr)
        }
    }
    ...
    private func handlePrepare() {
        ...
        do {
            ...
            fputs("[useful-keyboard] recorder prepare failed: \(error)\n", stderr)
            setState(.idle)
        }
    }
    ...
    private func handleStart() {
        ...
        if selectedBackend.backend == "nemotron" {
            recorder.cancel()
            fputs("[useful-keyboard] hold-to-talk blocked for Nemotron, showing warning\n", stderr)
            indicator.showWarning("Double-tap for Nemotron handsfree mode", icon: "⚡")
            return
        }

        fputs("[useful-keyboard] recording start\n", stderr)
        ...
        do {
            ...
            fputs("[useful-keyboard] recorder start failed: \(error)\n", stderr)
            setState(.idle)
        }
    }
    ...
    private func startNemotronStreamingAsync() {
        Task {
            ...
            fputs("[useful-keyboard] got Nemotron transcriber\n", stderr)
            ...
            controller.onPartialText = { [weak self] fullText in
                ...
                fputs("[useful-keyboard] streaming partial: +\"\(delta)\" (total \(fullText.count) chars)\n", stderr)
                if !delta.isEmpty {
            ...
            await MainActor.run {
                ...
                controller.start()
                fputs("[useful-keyboard] Nemotron streaming controller started\n", stderr)
            }
        }
    }
    ...
    private func handleToggleStart() {
        if isMeetingRecording() { return }
        fputs("[useful-keyboard] toggle dictation start\n", stderr)
        micActivityMonitor.suppressWhileActive()
        ...
        if selectedBackend.backend == "nemotron" {
            if #available(macOS 15, *) {
                ...
                fputs("[useful-keyboard] Nemotron streaming toggle mode active\n", stderr)
        ...
        do {
            ...
            fputs("[useful-keyboard] toggle start failed: \(error)\n", stderr)
            setState(.idle)
        }
    }

    private func handleToggleStop() {
        fputs("[useful-keyboard] toggle dictation stop\n", stderr)
    ...
    private func handleStop() {
        ...
        if isNemotronStreaming {
            ...
            if #available(macOS 15, *), let controller = _streamingDictationController as? StreamingDictationController {
                ...
                fputs("[useful-keyboard] Nemotron streaming stop, got \(finalText.count) chars\n", stderr)
            } else {
                fputs("[useful-keyboard] Nemotron streaming stop, controller not ready (short press)\n", stderr)
            }
            _streamingDictationController = nil
            previousStreamText = ""
            ...
            setState(.idle)
            micActivityMonitor.resumeAfterCooldown()
            fputs("[useful-keyboard] Nemotron streaming done (\(String(format: "%.1f", duration))s)\n", stderr)
            return
        }

        // Standard path: stop recording → transcribe → paste
        guard let wavURL = recorder.stop() else {
            fputs("[useful-keyboard] stop without wav\n", stderr)
            setState(.idle)
            return
        }
        let duration = max(Date().timeIntervalSince(startedAt), 0)
        if duration < 0.3 {
            fputs("[useful-keyboard] discarded short recording\n", stderr)
        ...
        Task { [weak self] in
            ...
            do {
                ...
                fputs("[useful-keyboard] transcription failed: \(error)\n", stderr)
                await MainActor.run {
                    self.setState(.idle)
                }
            }
    ...
    private func handleUpcomingMeeting(_ event: UpcomingMeetingEvent) {
        fputs("[useful-keyboard] meeting soon: \(event.title)\n", stderr)
