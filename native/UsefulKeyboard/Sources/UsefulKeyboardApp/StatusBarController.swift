...
final class StatusBarController: NSObject, NSMenuDelegate {
    ...
    init(controller: AppController, runtime: RuntimePaths) {
    ...
    private func rebuildMenu() {
        ...
        menu.addItem(actionItem(title: "Open \(AppIdentity.displayName)", action: #selector(AppController.openHistoryWindow as (AppController) -> () -> Void)))
        let meetingTitle = controller.isMeetingRecording() ? "Stop Meeting Recording" : "Start Meeting Recording"
        menu.addItem(actionItem(title: meetingTitle, action: #selector(AppController.toggleMeetingRecording)))
        if controller.isMeetingRecording() {
            menu.addItem(actionItem(title: "Discard Meeting Recording...", action: #selector(AppController.discardMeetingWithConfirmation)))
        }
        ...
        if recentRows.isEmpty {
            ...
            for row in recentRows {
                let item = NSMenuItem(title: controller.truncate(row.rawText, limit: 54), action: #selector(AppController.copyRecentDictation(_:)), keyEquivalent: "")
        ...
        if meetingRows.isEmpty {
            ...
            for row in meetingRows {
                ...
                let item = NSMenuItem(
                    ...
                    action: #selector(AppController.copyRecentMeeting(_:)),
                    keyEquivalent: ""
                )
                item.target = controller
        ...
        for option in BackendOption.downloaded {
            ...
            let item = NSMenuItem(title: "\(prefix)\(option.label)", action: #selector(AppController.selectBackendFromMenu(_:)), keyEquivalent: "")
            item.target = controller
        ...
        for option in MeetingSummaryBackendOption.all {
            ...
            let item = NSMenuItem(
                ...
                action: #selector(AppController.selectMeetingSummaryBackendFromMenu(_:)),
                keyEquivalent: ""
            )
            item.target = controller
            item.representedObject = option.label
        ...
        menu.addItem(.separator())
        menu.addItem(actionItem(title: "Settings…", action: #selector(AppController.openSettingsTab)))
        statusLabel.isEnabled = false
        menu.addItem(statusLabel)
        menu.addItem(.separator())
        menu.addItem(actionItem(title: "Quit", action: #selector(AppController.quitApp)))
    }
