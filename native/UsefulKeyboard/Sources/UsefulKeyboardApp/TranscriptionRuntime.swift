...
actor TranscriptionCoordinator {
    ...
    func preload(backend: BackendOption, progress: ((Double, String?) -> Void)? = nil) async {
        ...
        if vadManager == nil {
            do {
                ...
                fputs("[useful-keyboard] Silero VAD loaded\n", stderr)
            } catch {
                fputs("[useful-keyboard] VAD load failed (non-critical): \(error)\n", stderr)
            }
        }
        ...
        if diarizerManager == nil {
            do {
                ...
                fputs("[useful-keyboard] Speaker diarization loaded\n", stderr)
            } catch {
                fputs("[useful-keyboard] Diarization load failed (non-critical): \(error)\n", stderr)
            }
        }
        ...
        case "fluidaudio":
            ...
            do {
                ...
                fputs("[useful-keyboard] FluidAudio preload failed: \(error)\n", stderr)
            }
        case "whisper":
            do {
                try await whisperTranscriber.loadModel(modelName: backend.model, progress: progress)
            } catch {
                fputs("[useful-keyboard] whisper.cpp preload failed: \(error)\n", stderr)
            }
        case "nemotron":
            if #available(macOS 15, *) {
                do {
                    ...
                    fputs("[useful-keyboard] Nemotron warmup: running silent chunk for ANE compilation...\n", stderr)
                    ...
                    fputs("[useful-keyboard] Nemotron warmup complete\n", stderr)
                } catch {
                    fputs("[useful-keyboard] Nemotron preload failed: \(error)\n", stderr)
                }
            } else {
                fputs("[useful-keyboard] Nemotron requires macOS 15+\n", stderr)
            }
        case "qwen":
            if #available(macOS 15, *) {
                do {
                    ...
                    fputs("[useful-keyboard] Qwen3 ASR preload failed: \(error)\n", stderr)
                }
            } else {
                fputs("[useful-keyboard] Qwen3 ASR requires macOS 15+\n", stderr)
            }
        case "canary":
            if #available(macOS 15, *) {
                do {
                    ...
                    fputs("[useful-keyboard] Canary Qwen preload failed: \(error)\n", stderr)
                }
            } else {
                fputs("[useful-keyboard] Canary Qwen requires macOS 15+\n", stderr)
            }
        case "cohere":
            if #available(macOS 15, *) {
                do {
                    ...
                    fputs("[useful-keyboard] Cohere Transcribe preload failed: \(error)\n", stderr)
                }
            } else {
                fputs("[useful-keyboard] Cohere Transcribe requires macOS 15+\n", stderr)
            }
        default:
            fputs("[useful-keyboard] unknown backend: \(backend.backend)\n", stderr)
        }
    }
    ...
    func transcribeDictation(at url: URL, backend: BackendOption, customWords: [[String: Any]] = []) async throws -> SpeechTranscriptionResult {
        ...
        if backend.backend == "cohere", let vadManager {
            do {
                ...
                if !hasSpeech {
                    fputs("[useful-keyboard] VAD: dictation is silent, skipping Cohere transcription\n", stderr)
                    return SpeechTranscriptionResult(text: "", segments: [])
                }
            } catch {
                fputs("[useful-keyboard] VAD check failed, transcribing anyway: \(error)\n", stderr)
            }
        }
    ...
    func transcribeMeetingChunk(at url: URL, backend: BackendOption, customWords: [[String: Any]] = []) async throws -> SpeechTranscriptionResult {
        ...
        if let vadManager {
            do {
                ...
                let hasSpeech = vadResults.contains { $0.probability > 0.5 }
                if !hasSpeech {
                    fputs("[useful-keyboard] VAD: chunk is silent, skipping transcription\n", stderr)
                    return SpeechTranscriptionResult(text: "", segments: [])
                }
            } catch {
                fputs("[useful-keyboard] VAD check failed, transcribing anyway: \(error)\n", stderr)
    ...
    func diarizeSystemAudio(at url: URL) async throws -> DiarizationResult? {
        guard let diarizerManager, diarizerManager.isAvailable else {
            fputs("[useful-keyboard] diarization not available, skipping\n", stderr)
            return nil
        }
        fputs("[useful-keyboard] running speaker diarization on system audio...\n", stderr)
        ...
        fputs("[useful-keyboard] diarization complete: \(result.segments.count) segments, \(speakerCount) speakers\n", stderr)
        return result
    }
    ...
    private func transcribeWithFluidAudio(url: URL) async throws -> SpeechTranscriptionResult {
        fputs("[useful-keyboard] transcribing with FluidAudio: \(url.lastPathComponent)\n", stderr)
        let result = try await fluidTranscriber.transcribe(wavURL: url)
        fputs("[useful-keyboard] FluidAudio result: \(result.text.prefix(80)) (took \(String(format: "%.3f", result.processingTime))s)\n", stderr)
    ...
    private func transcribeWithWhisperCpp(url: URL) async throws -> SpeechTranscriptionResult {
        fputs("[useful-keyboard] transcribing with whisper.cpp: \(url.lastPathComponent)\n", stderr)
        let result = try await whisperTranscriber.transcribe(wavURL: url)
        fputs("[useful-keyboard] whisper.cpp result: \(result.text.prefix(80)) (took \(String(format: "%.3f", result.processingTime))s)\n", stderr)
        let text = result.text.trimmingCharacters(in: .whitespacesAndNewlines)
        return SpeechTranscriptionResult(
            text: text,
    ...
    private func transcribeWithQwen3(url: URL) async throws -> SpeechTranscriptionResult {
        if #available(macOS 15, *) {
            fputs("[useful-keyboard] transcribing with Qwen3 ASR: \(url.lastPathComponent)\n", stderr)
            let result = try await qwen3Transcriber.transcribe(wavURL: url)
            fputs("[useful-keyboard] Qwen3 ASR result: \(result.text.prefix(80)) (took \(String(format: "%.3f", result.processingTime))s)\n", stderr)
            let text = result.text.trimmingCharacters(in: .whitespacesAndNewlines)
            return SpeechTranscriptionResult(
                text: text,
                segments: text.isEmpty ? [] : [SpeechSegment(start: 0, end: 0, text: text)]
            ...
            throw NSError(domain: "UsefulKeyboard", code: 1, userInfo: [
    ...
    private func transcribeWithCanaryQwen(url: URL) async throws -> SpeechTranscriptionResult {
        if #available(macOS 15, *) {
            fputs("[useful-keyboard] transcribing with Canary Qwen: \(url.lastPathComponent)\n", stderr)
            let result = try await canaryQwenTranscriber.transcribe(wavURL: url)
            fputs("[useful-keyboard] Canary Qwen result: \(result.text.prefix(80)) (took \(String(format: "%.3f", result.processingTime))s)\n", stderr)
            CanaryProfilingLog.write("[useful-keyboard] Canary Qwen profile: \(result.profile.logDescription(prefix: "profile"))")
            let text = result.text.trimmingCharacters(in: .whitespacesAndNewlines)
            return SpeechTranscriptionResult(
                text: text,
                segments: text.isEmpty ? [] : [SpeechSegment(start: 0, end: 0, text: text)]
    ...
    private func transcribeWithCohere(url: URL) async throws -> SpeechTranscriptionResult {
        if #available(macOS 15, *) {
            fputs("[useful-keyboard] transcribing with Cohere Transcribe: \(url.lastPathComponent)\n", stderr)
            let result = try await cohereTranscriber.transcribe(wavURL: url)
            fputs("[useful-keyboard] Cohere Transcribe result: \(result.text.prefix(80)) (took \(String(format: "%.3f", result.processingTime))s)\n", stderr)
            let text = result.text.trimmingCharacters(in: .whitespacesAndNewlines)
            return SpeechTranscriptionResult(
                text: text,
                segments: text.isEmpty ? [] : [SpeechSegment(start: 0, end: 0, text: text)]
    ...
    private func transcribeWithNemotron(url: URL) async throws -> SpeechTranscriptionResult {
        if #available(macOS 15, *) {
            fputs("[useful-keyboard] transcribing with Nemotron: \(url.lastPathComponent)\n", stderr)
            let result = try await nemotronTranscriber.transcribe(wavURL: url)
            fputs("[useful-keyboard] Nemotron result: \(result.text.prefix(80)) (took \(String(format: "%.3f", result.processingTime))s)\n", stderr)
            let text = result.text.trimmingCharacters(in: .whitespacesAndNewlines)
            return SpeechTranscriptionResult(
                text: text,
                segments: text.isEmpty ? [] : [SpeechSegment(start: 0, end: 0, text: text)]
