...
struct CLIContext {
    ...
    init(dbPath: String?, supportDir: String?) {
        ...
        if let supportDir, !supportDir.isEmpty {
            ...
            self.databaseURL = self.supportDirectory.appendingPathComponent("useful-keyboard.db")
            return
        }

        if let envDB = ProcessInfo.processInfo.environment["UK_DB_PATH"], !envDB.isEmpty {
            self.databaseURL = URL(fileURLWithPath: envDB)
            self.supportDirectory = URL(fileURLWithPath: ProcessInfo.processInfo.environment["UK_SUPPORT_DIR"] ?? self.databaseURL.deletingLastPathComponent().path)
            return
        }

        if let envSupport = ProcessInfo.processInfo.environment["UK_SUPPORT_DIR"], !envSupport.isEmpty {
            self.supportDirectory = URL(fileURLWithPath: envSupport)
            self.databaseURL = self.supportDirectory.appendingPathComponent("useful-keyboard.db")
            return
        }

        self.supportDirectory = AppPaths.defaultSupportDirectoryURL()
        self.databaseURL = self.supportDirectory.appendingPathComponent("useful-keyboard.db")
    }
...
func appBundlePath() -> String? {
    ...
    let defaultPath = "/Applications/Useful Keyboard.app"
...
func ensureDatabaseAvailable(_ context: CLIContext, command: String) throws {
    guard context.store.databaseExists else {
        throw CLIError.databaseUnavailable(
            "No database exists at the resolved path.",
            fix: "Launch Useful Keyboard once or pass --db-path/--support-dir to point at the correct data directory."
        )
    }
}

struct GlobalOptions: ParsableArguments {
    @Option(name: .long, help: "Override the absolute path to useful-keyboard.db.")
    var dbPath: String?

    @Option(name: .long, help: "Override the Useful Keyboard support directory. The CLI will look for useful-keyboard.db inside it.")
...
@main
struct UsefulKeyboardCLI: ParsableCommand {
    static let configuration = CommandConfiguration(
        commandName: "useful-keyboard-cli",
        abstract: "Agent-friendly CLI for local Useful Keyboard meetings and dictations.",
    ...
    static func exit(withError error: Error? = nil) -> Never {
        ...
        if let validation = error as? ValidationError {
            emitFailure(
                ...
                error: ErrorBody(code: "invalid_input", message: validation.message, fix: "Run `useful-keyboard-cli spec` for valid usage."),
                dbPath: nil
            )
            Foundation.exit(4)
        }
        ...
        emitFailure(
            command: CommandLine.arguments.joined(separator: " "),
            error: ErrorBody(code: "usage_error", message: error.localizedDescription, fix: "Run `useful-keyboard-cli spec` for valid usage."),
            dbPath: nil
        )
    ...
    func run() throws {
        ...
        emitSuccess(command: "useful-keyboard-cli", data: payload, dbPath: CLIContext(options: .init()).databaseURL)
    }

    static func specPayload() -> CommandSpecPayload {
        CommandSpecPayload(commands: [
            .init(name: "spec", usage: "useful-keyboard-cli spec", summary: "Dump the command tree and CLI schema metadata.", examples: ["useful-keyboard-cli spec"]),
            .init(name: "info", usage: "useful-keyboard-cli info [--db-path <path>] [--support-dir <dir>]", summary: "Show resolved support and database paths.", examples: ["useful-keyboard-cli info", "useful-keyboard-cli info --support-dir ~/Library/Application\\ Support/Useful Keyboard"]),
            .init(name: "meetings list", usage: "useful-keyboard-cli meetings list [--limit <n>] [--folder-id <id>]", summary: "List recent meetings.", examples: ["useful-keyboard-cli meetings list --limit 5", "useful-keyboard-cli meetings list --folder-id 2"]),
            .init(name: "meetings get", usage: "useful-keyboard-cli meetings get <id>", summary: "Return a full meeting record.", examples: ["useful-keyboard-cli meetings get 42"]),
            .init(name: "meetings update-notes", usage: "useful-keyboard-cli meetings update-notes <id> (--stdin | --file <path>)", summary: "Replace stored meeting notes only.", examples: ["useful-keyboard-cli meetings update-notes 42 --file notes.md", "cat notes.md | useful-keyboard-cli meetings update-notes 42 --stdin"]),
            .init(name: "dictations list", usage: "useful-keyboard-cli dictations list [--limit <n>]", summary: "List recent dictations.", examples: ["useful-keyboard-cli dictations list --limit 10"]),
            .init(name: "dictations get", usage: "useful-keyboard-cli dictations get <id>", summary: "Return a full dictation record.", examples: ["useful-keyboard-cli dictations get 7"]),
        ])
    }
}
...
struct SpecCommand: ParsableCommand {
    ...
    func run() throws {
        ...
        emitSuccess(command: "useful-keyboard-cli spec", data: UsefulKeyboardCLI.specPayload(), dbPath: context.databaseURL)
    }
}
...
struct InfoCommand: ParsableCommand {
    ...
    func run() throws {
        let context = CLIContext(options: global)
        emitSuccess(
            command: "useful-keyboard-cli info",
            data: Payload(
...
struct MeetingsCommand: ParsableCommand {
    static let configuration = CommandConfiguration(commandName: "meetings", abstract: "Inspect and update Useful Keyboard meetings.", subcommands: [MeetingsListCommand.self, MeetingsGetCommand.self, MeetingsUpdateNotesCommand.self])
}
...
struct MeetingsListCommand: ParsableCommand {
    ...
    func run() throws {
        ...
        if !context.store.databaseExists {
            emitSuccess(command: "useful-keyboard-cli meetings list", data: [MeetingListRow](), dbPath: context.databaseURL, warnings: ["No database exists at the resolved path."])
            return
        }
        let rows = try context.store.recentMeetings(limit: limit, folderID: folderID).map(MeetingListRow.init)
        emitSuccess(command: "useful-keyboard-cli meetings list", data: rows, dbPath: context.databaseURL)
    }
}
...
struct MeetingsGetCommand: ParsableCommand {
    ...
    func run() throws {
        let context = CLIContext(options: global)
        try ensureDatabaseAvailable(context, command: "useful-keyboard-cli meetings get")
        guard let meeting = try context.store.meeting(id: id) else {
            throw CLIError.notFound("No meeting exists with id \(id).", fix: "Run `useful-keyboard-cli meetings list` to find a valid ID.")
        }
        emitSuccess(command: "useful-keyboard-cli meetings get", data: MeetingDetailPayload(meeting), dbPath: context.databaseURL)
    }
}
...
struct MeetingsUpdateNotesCommand: ParsableCommand {
    ...
    func run() throws {
        let context = CLIContext(options: global)
        try ensureDatabaseAvailable(context, command: "useful-keyboard-cli meetings update-notes")
        ...
        emitSuccess(command: "useful-keyboard-cli meetings update-notes", data: MeetingDetailPayload(updated), dbPath: context.databaseURL)
    }
}

struct DictationsCommand: ParsableCommand {
    static let configuration = CommandConfiguration(commandName: "dictations", abstract: "Inspect Useful Keyboard dictations.", subcommands: [DictationsListCommand.self, DictationsGetCommand.self])
}
...
struct DictationsListCommand: ParsableCommand {
    ...
    func run() throws {
        ...
        if !context.store.databaseExists {
            emitSuccess(command: "useful-keyboard-cli dictations list", data: [DictationListRow](), dbPath: context.databaseURL, warnings: ["No database exists at the resolved path."])
            return
        }
        let rows = try context.store.recentDictations(limit: limit).map(DictationListRow.init)
        emitSuccess(command: "useful-keyboard-cli dictations list", data: rows, dbPath: context.databaseURL)
    }
}
...
struct DictationsGetCommand: ParsableCommand {
    ...
    func run() throws {
        let context = CLIContext(options: global)
        try ensureDatabaseAvailable(context, command: "useful-keyboard-cli dictations get")
        guard let dictation = try context.store.dictation(id: id) else {
            throw CLIError.notFound("No dictation exists with id \(id).", fix: "Run `useful-keyboard-cli dictations list` to find a valid ID.")
        }
        emitSuccess(command: "useful-keyboard-cli dictations get", data: DictationDetailPayload(dictation), dbPath: context.databaseURL)
    }
}
