...
struct CanaryQwenBackendTests {
    ...
    func localOverrideDirectory() async throws {
        guard let raw = ProcessInfo.processInfo.environment["UK_CANARY_MODEL_DIR"], !raw.isEmpty else {
            return
        }
    ...
    func loadModelsFromOverride() async throws {
        guard ProcessInfo.processInfo.environment["UK_CANARY_MODEL_DIR"] != nil else {
            return
        }
