...
@testable import UsefulKeyboardCLI

@Suite("UsefulKeyboardCLI", .serialized)
struct UsefulKeyboardCLITests {
    @Test("spec exposes the agent-facing command set")
    func specPayloadIncludesCommands() {
        let names = Set(UsefulKeyboardCLI.specPayload().commands.map(\.name))
    ...
    func cliContextUsesExplicitDatabasePath() {
        let context = CLIContext(
            dbPath: "/tmp/custom-uk.db",
            supportDir: "/tmp/ignored-support"
        )

        #expect(context.databaseURL.path == "/tmp/custom-uk.db")
    ...
    func cliContextUsesExplicitSupportDirectory() {
        let context = CLIContext(
            dbPath: nil,
            supportDir: "/tmp/uk-support"
        )

        #expect(context.supportDirectory.path == "/tmp/uk-support")
        #expect(context.databaseURL.path == "/tmp/uk-support/useful-keyboard.db")
    }
